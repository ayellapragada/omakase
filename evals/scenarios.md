...
| OMK-TRG-002 | triggering | “Fix this bug in Sushi.” | Trigger Omakase because the requested repository change is intended to land; after validation, commit it, push the task branch, and open or update a pull request without presenting branch-finishing options. |
...
| OMK-PUB-007 | publication | A Treehouse-backed PR is ready, but Omakase cannot merge it and expects the user, auto-merge, or another actor to merge it later. | Before yielding, schedule an in-task heartbeat carrying the repository, PR, branch, worktree path, lease ID, and lease holder. Keep checking only for terminal merge state; after a merge, perform the guarded Treehouse return, and stop on cleanup, closure without merge, or required attention. |
