...
If required checks or mergeability are pending, schedule a follow-up in the current Codex task carrying the repository and PR URL, branch and worktree, pending conditions, completed validation, and next permitted action. Concrete blockers such as conflicts, failed checks, draft status, or unsatisfied review requirements must be reported and handled or escalated as appropriate. Stop this pending-status follow-up when the PR closes or merges, attention is required, or the PR becomes ready. When readiness leaves an Omakase-acquired Treehouse lease awaiting an expected external merge, replace the pending-status follow-up with the lifecycle heartbeat below instead of stopping all monitoring.

If a Treehouse-backed PR is ready but must remain open because Omakase is not authorized to merge it and the user, auto-merge, or another actor is expected to merge it later, schedule an in-task heartbeat before yielding. Carry the repository and PR URL, branch and worktree path, `lease_id`, `lease_holder`, completed validation, and the guarded return procedure. On each run, refresh the PR only for a terminal outcome: after merge, verify and return the allocation through [the Treehouse lifecycle](references/treehouse-worktrees.md); after closure without merge or any dirty, live-process, or identity mismatch, preserve the lease and request attention. Stop after cleanup or attention. Expected external merge makes this lifecycle follow-up useful; it does not authorize Omakase to perform the merge. Do not create this heartbeat when Omakase completes an authorized merge in the active task or when no Omakase-acquired Treehouse lease exists.
